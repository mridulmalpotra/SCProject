"""Main function to render the views in CollegeEventsApp"""
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if user.check_password(password):
                ...
                return render_to_response('login_page.html', {'msg' : "Invalid Login"}, \
                    context_instance=RequestContext(request))
        except ObjectDoesNotExist:
            return render_to_response('login_page.html', {'msg' : "This user doesn't exist"},\
             context_instance=RequestContext(request))
...
def signup_view(request):
    if request.method == 'GET':
        ...
        if password != password_confirmation:
            return render_to_response('signup.html', {'msg' : "Passwords doesn't match"}, \
                context_instance=RequestContext(request))
        else:
...
def user_view(request):
    if request.user.is_authenticated():
        ...
        if user.is_superuser:
            ...
            return render_to_response('admin_all_users.html', {'data' : user}, \
                context_instance=RequestContext(request))
        else:
            return HttpResponseRedirect('/eventsApp/dashboard')
...
def profile(request):
    if request.user.is_authenticated():
        if request.method == 'GET':
            ...
            return render_to_response('profile.html', {'data' : profile}, \
                context_instance=RequestContext(request))
        if request.method == 'POST':
            ...
            if request.POST.get('test', False):
                return render_to_response('change_password.html', {'data': profile}, \
                    context_instance=RequestContext(request))
            password = request.POST['password']
            password_confirmation = request.POST['password_confirmation']
            if password != password_confirmation:
...
def create_event(request):
    if request.user.is_authenticated():
        ...
        if request.method == 'POST':
            ...
            user = User.objects.get(username=request.user)
            profile = UserProfile.objects.get(user=user.pk)
            event = Events.objects.create(userWhoPosted=profile, eventName=eventName, \
                eventDescription=eventDescription, eventDate=date, eventTime=time)
            event.save()
            profile.save()
            return HttpResponseRedirect('/eventsApp/dashboard')
    else:
...
def view_all_events(request):
    if request.user.is_authenticated():
        if request.method == 'GET':
            ...
            user = User.objects.get(username=request.user)
            if user.is_superuser:
                return render_to_response('admin_all_events.html', {'data' : event}, \
                    context_instance=RequestContext(request))
            else:
                return render_to_response('view_all_events.html', {'data' : event}, \
                    context_instance=RequestContext(request))
        if request.method == 'POST':
            user = User.objects.get(username=request.user)
            if user.is_superuser:
                if request.POST.get('eventId', False):
                    ...
                    if delete:
                        if request.POST.get('confirm', False):
                            ...
                            return render_to_response('confirmation.html', {'data' : event}, \
                                context_instance=RequestContext(request))
                    else:
                        if request.POST.get('editFin', False):
                            ...
                            Events.objects.filter(eventid=eventid).update(eventDescription=\
                                eventDescription, eventDate=eventDate, eventTime=eventTime)
                            event = Events.objects.all()
                            return render_to_response('admin_all_events.html', {'data' : event}, \
                                context_instance=RequestContext(request))
                        else:
                            event = Events.objects.get(eventid=eventid)
                            return render_to_response('edit_event.html', {'data' : event}, \
                                context_instance=RequestContext(request))
            else:
                return HttpResponseRedirect('/eventsApp/dashboard')
    else:
...
def view_my_events(request):
    if request.user.is_authenticated():
        if request.method == 'GET':
            ...
            return render_to_response('my_event.html', {'data' : event}, \
                context_instance=RequestContext(request))
        if request.method == 'POST':
            if request.POST.get('eventId', False):
                eventid = request.POST['eventId']
...
def eventsUnsafeUserAPI(request, username):
    if request.method == 'GET':
        try:
            events = Events.objects.raw('SELECT * FROM CollegeEventsApp_events where \
                userWhoPosted_id=\"' + username + '\"')
            data = "["
            for e in events:
                print str(e)
                data += str(e) + ","
